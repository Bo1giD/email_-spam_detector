...
from scipy.sparse._csr import csr_matrix as CSRMatrix 
...
def contains_phishing_keywords(text: str) -> bool:
    """Check if the given text contains phishing-related keywords."""
    text = text.lower()
    return any(keyword in text for keyword in PHISHING_KEYWORDS)

def extract_binary_features(text: str) -> List[int]:
...
def predict_spam(email_input: Dict[str, str]) -> Tuple[str, float]:
    ...
    """
    full_text: str = f"{email_input['sender']} {email_input['subject']} {email_input['body']}"
    ...
    # Extract features
    binary_features: List[int] = extract_binary_features(full_text)
    binary_array: np.ndarray = np.array(binary_features).reshape(1, -1)
    binary_sparse: CSRMatrix = csr_matrix(binary_array)
    ...
    # Predict probability
    prob_spam: float = model.predict_proba(combined)[0][1]
